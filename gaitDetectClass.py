...
class gaitDetect:
    ...
    def testVal(self, thigh, shank, heel):
        import time
        import numpy as np
        if(thigh==None or shank==None or heel==None):
            self.gaitStage=0
        else:
...
#### Vaibhav - 06 April 2023 #######

#detects negative to positive, aka Heel strike or start of stance phase
#            if self.movingAvgHeel > 0 and self.lastAvgHeel < 0 and self.gaitStage == 1:
 #               self.significance = 1
  #              self.timeLastToeOff = time.time()
   #             self.gaitStage = 0

#detects positive to negative, aka toeoff  or start of swing phase
    #        elif self.movingAvgHeel < 0 and self.lastAvgHeel > 0 and self.gaitStage == 0: 
     #           self.significance = -1
      #          self.timeLastHeelStrike = time.time()
       #         self.gaitStage = 1
...
#When significance has been changed, wait the appropriate amount of time before resetting to allow for next gait update
            ...
            self.lastAvgHeel = self.movingAvgHeel
            if (self.gaitStage == 2):
                self.gaitOutput = 1
            else:
                self.gaitOutput = 0
