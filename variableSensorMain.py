...
def data_handler(address, *args):
    ...
    # trip trigger algo variables
    global step_num, initial_gait_state, current_gait_state
    ###########################################################################################
    ...
    ###########################################################################################
        #DATA OUTPUT (FILE) -------------------------------------------------------------------------------------------------------------
	#ALL CALCS HAVE BEEN COMPLETED BY THIS POINT
        ...
        #/////////////////////////////////////////////////####################################
        #read gait stager here and count number of steps taken ignoring the first
	#  after heel_strike event gaitstage = 0
        #  after heel_off event gaitstage = 1
        #  after toe_off event gaitstage = 2
        

        # at the start initialize gait_state
        #if (time.time()-timeStart)<1:
        #    initial_gait_state = gatDetectRight.gaitStage
        #current_gait_state = gatDetectRight.gaitStage
        # trip triggering algorithm starts here
        #if ((initial_gait_state==2) & (current_gait_state==0)):
        #    step_num +=step_num
    ...
    ###########################################################################################
        ...
        print(f"{123}\t{objLowBack.acX}\t{objLowBack.acY}\t{objLowBack.acZ}\t{np.round(objLowBack.gyX,2)}\t{np.round(objLowBack.gyY,2)}\t{np.round(objLowBack.gyZ,5)}")
        print(f"{ttr}\t{objLowBack.acX}\t{objLowBack.acY}\t{objLowBack.acZ}\t{np.round(objLowBack.gyX,2)}\t{np.round(objLowBack.gyY,2)}\t{np.round(objLThigh.gyZ,5)}")
        print(f"{ttr}\t{objLowBack.acX}\t{objLowBack.acY}\t{objLowBack.acZ}\t{np.round(objLowBack.gyX,2)}\t{np.round(objLowBack.gyY,2)}\t{np.round(objLShank.gyZ,5)}")
        print(f"{ttr}\t{objLowBack.acX}\t{objLowBack.acY}\t{objLowBack.acZ}\t{np.round(objLowBack.gyX,2)}\t{np.round(objLowBack.gyY,2)}\t{np.round(objLHeel.gyZ,5)}")
